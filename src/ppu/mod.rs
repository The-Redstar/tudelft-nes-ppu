...
impl Ppu {
    ...
    pub fn write_ppu_register(&mut self, cpu: &mut impl Cpu, register: PpuRegister, value: u8) {
        self.bus = value;
        ...
        match register {
            ...
            PpuRegister::Data => {
                match self.addr.addr {
                    a @ 0..=0x1fff => {
                        //log::debug!("write to read-only part of memory (chr rom) through ppu data register: 0x{a:0x}"),
                        cpu.ppu_memory_write(a, value);
                    }
                    a @ 0x2000..=0x2fff => {
